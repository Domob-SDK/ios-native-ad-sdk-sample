...
@interface PhotoFlowViewController ()<RefreshControlDelegate,DMNativeAdDelegate,PhotoFlowTableViewCellDelegate>
...
@implementation PhotoFlowViewController
...
- (void)createNativeAd
{
    ...
    self.nativeAd = [[DMNativeAd alloc] initWithPublisherId:PUBLIHSER_ID
                                                placementId:PHOTOFLOW_PLACEMENT_ID
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return 100 * (ScreenSize.width - 100)/(3*74) + 20.f;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    
    static NSString *cellIdentifier = @"cellIdentifier";
    PhotoFlowTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell == nil) {
        
        cell = [[[NSBundle mainBundle] loadNibNamed:@"PhotoFlowTableViewCell" owner:nil options:nil] objectAtIndex:0];
        
        UINib *nib = [UINib nibWithNibName:@"PhotoFlowTableViewCell" bundle:[NSBundle mainBundle]];
...
#pragma mark -- PhotoFlowViewCellDelegate
- (void)cellDidClick:(NSInteger)tag
{
    id model = [self.dataArray objectAtIndex:tag];
    
    if ([model isKindOfClass:[DMNativeAd class]]) {
#warning DMNativeAd的点击事件处理
        //DMNativeAd的点击事件
        [model processClickAction];
    }
}
